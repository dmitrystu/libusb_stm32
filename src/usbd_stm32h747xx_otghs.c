...
inline static void Flush_TX(uint8_t ep) {
    _BMD(OTG->GRSTCTL, USB_OTG_GRSTCTL_TXFNUM, _VAL2FLD(USB_OTG_GRSTCTL_TXFNUM, ep) | USB_OTG_GRSTCTL_TXFFLSH);
...
static uint32_t getinfo(void) {
    ...
    if (!(OTGD->DCTL & USB_OTG_DCTL_SDIS)) {
        if (_FLD2VAL(USB_OTG_DSTS_ENUMSPD, OTGD->DSTS) == 0x00)
            return STATUS_VAL(USBD_HW_ENABLED | USBD_HW_SPEED_HS);
    ...
    if (ep & 0x80) {
        ...
        if (_t & USB_OTG_DIEPCTL_USBAEP) {
            if (stall) {
                ...
                _BMD(_t, USB_OTG_DIEPCTL_STALL, USB_OTG_DIEPCTL_SD0PID_SEVNFRM | USB_OTG_DIEPCTL_CNAK);
            }
            EPIN(ep)->DIEPCTL = _t;
        }
    } else {
...
static void enable(bool enable) {
    if (enable) {
        ...
        _BST(RCC->AHB1ENR, RCC_AHB1ENR_USB1OTGHSULPIEN);
#endif
        /* waiting AHB ready */
        _WBS(OTG->GRSTCTL, USB_OTG_GRSTCTL_AHBIDL);
        /* configure OTG as device */
#ifdef USBD_ULPI
        OTG->GUSBCFG = USB_OTG_GUSBCFG_FDMOD | _VAL2FLD(USB_OTG_GUSBCFG_TRDT, 0x09) |
                       _VAL2FLD(USB_OTG_GUSBCFG_TOCAL, 0x01);

        _BCL(OTG->GUSBCFG,
             USB_OTG_GUSBCFG_TSDPS | USB_OTG_GUSBCFG_ULPIFSLS | USB_OTG_GUSBCFG_PHYSEL | USB_OTG_GUSBCFG_ULPIEVBUSD |
                 USB_OTG_GUSBCFG_ULPIEVBUSI);
#else
        OTG->GUSBCFG = USB_OTG_GUSBCFG_FDMOD | USB_OTG_GUSBCFG_PHYSEL | _VAL2FLD(USB_OTG_GUSBCFG_TRDT, 0x09) |
                       _VAL2FLD(USB_OTG_GUSBCFG_TOCAL, 0x01);
#endif
        ...
        *OTGPCTL = 0;
        ...
        OTG->GINTMSK = USB_OTG_GINTMSK_USBRST | USB_OTG_GINTMSK_ENUMDNEM |
                       ...
                       USB_OTG_GINTMSK_EOPFM | USB_OTG_GINTMSK_USBSUSPM | USB_OTG_GINTMSK_WUIM |
...
#ifdef USBD_ULPI
            _BCL(RCC->AHB1ENR, RCC_AHB1ENR_USB1OTGHSULPIEN);
#endif
...
static void setaddr(uint8_t addr) { _BMD(OTGD->DCFG, USB_OTG_DCFG_DAD, addr << 4); }
...
static bool ep_config(uint8_t ep, uint8_t eptype, uint16_t epsize) {
    ...
    if (ep & 0x80) {
        ...
        case USB_EPTYPE_ISOCHRONUS | USB_EPTYPE_DBLBUF:
            epi->DIEPCTL = (0x01 << 18) | USB_OTG_DIEPCTL_USBAEP | USB_OTG_DIEPCTL_SD0PID_SEVNFRM | (ep << 22) | epsize;
            break;
        case USB_EPTYPE_BULK:
        case USB_EPTYPE_BULK | USB_EPTYPE_DBLBUF:
            epi->DIEPCTL = USB_OTG_DIEPCTL_SNAK | USB_OTG_DIEPCTL_USBAEP | (0x02 << 18) |
                           USB_OTG_DIEPCTL_SD0PID_SEVNFRM | (ep << 22) | epsize;
            break;
        default:
            epi->DIEPCTL = USB_OTG_DIEPCTL_SNAK | USB_OTG_DIEPCTL_USBAEP | (0x03 << 18) |
                           USB_OTG_DIEPCTL_SD0PID_SEVNFRM | (ep << 22) | epsize;
            break;
        }
    } else {
        ...
        case USB_EPTYPE_ISOCHRONUS:
        case USB_EPTYPE_ISOCHRONUS | USB_EPTYPE_DBLBUF:
            epo->DOEPCTL = USB_OTG_DOEPCTL_CNAK | USB_OTG_DOEPCTL_EPENA | USB_OTG_DOEPCTL_USBAEP | (0x01 << 18) |
                           epsize;
            break;
        case USB_EPTYPE_BULK | USB_EPTYPE_DBLBUF:
        case USB_EPTYPE_BULK:
            epo->DOEPCTL = USB_OTG_DOEPCTL_SD0PID_SEVNFRM | USB_OTG_DOEPCTL_CNAK | USB_OTG_DOEPCTL_EPENA |
                           USB_OTG_DOEPCTL_USBAEP | (0x02 << 18) | epsize;
            break;
        default:
            epo->DOEPCTL = USB_OTG_DOEPCTL_SD0PID_SEVNFRM | USB_OTG_DOEPCTL_CNAK | USB_OTG_DOEPCTL_EPENA |
                           USB_OTG_DOEPCTL_USBAEP | (0x03 << 18) | epsize;
            break;
        }
    }
    return true;
...
static int32_t ep_write(uint8_t ep, const void* buf, uint16_t blen) {
    ...
    if (len > _FLD2VAL(USB_OTG_DTXFSTS_INEPTFSAV, epi->DTXFSTS))
        return -1;
...
static uint16_t get_frame(void) { return _FLD2VAL(USB_OTG_DSTS_FNSOF, OTGD->DSTS); }
...
static void evt_poll(usbd_device* dev, usbd_evt_callback callback) {
    ...
    while (1) {
        ...
        if (_t & USB_OTG_GINTSTS_USBRST) {
            ...
            for (uint8_t i = 0; i < MAX_EP; i++) { ep_deconfig(i); }
            Flush_RX();
            continue;
