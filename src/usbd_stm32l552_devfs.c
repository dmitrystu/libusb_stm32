...
#if defined(USBD_STM32L552)

#define USB_PMAADDR USB_PMAADDR_NS
...
static uint32_t getinfo(void) {
    if (!(RCC->APB1ENR2 & RCC_APB1ENR2_USBFSEN)) return STATUS_VAL(0);
...
static void enable(bool enable) {
    if (enable) {
        RCC->APB1ENR2  |=  RCC_APB1ENR2_USBFSEN;
        RCC->APB1RSTR2 |= RCC_APB1RSTR2_USBFSRST;
        RCC->APB1RSTR2 &= ~RCC_APB1RSTR2_USBFSRST;
    ...
    } else if (RCC->APB1ENR2 & RCC_APB1ENR2_USBFSEN) {
        USB->BCDR = 0;
        RCC->APB1RSTR2 |= RCC_APB1RSTR2_USBFSRST;
        RCC->APB1ENR2 &= ~RCC_APB1ENR2_USBFSEN;
    }
